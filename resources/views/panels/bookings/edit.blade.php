...
@section('content')
    <div class="d-flex justify-content-between align-items-start mb-4">
        <div><h2 class="mb-1">Review Booking #{{ $booking->id }}</h2><p class="text-muted mb-0">Client: {{ $booking->user->name }}</p></div>
    ...
    <form method="POST" action="{{ route('business.bookings.update', $booking) }}">@csrf @method('PUT')
        <div class="row g-4">
            ...
            <div class="col-lg-4">
                ...
                @if($booking->rental_type === 'self_drive')<div class="card"><div class="card-body"><h5>Client Profile & Self-Drive Requirements</h5>@php($profile = $booking->user->clientProfile)<p class="mb-1"><strong>Name:</strong> {{ $profile?->full_name ?? 'Not completed' }}</p><p class="mb-1"><strong>Address:</strong> {{ $profile?->permanent_address ?? 'Not completed' }}</p><p class="mb-3"><strong>Contact:</strong> {{ $profile?->email_address ?? $booking->user->email }}</p><p class="mb-0"><strong>Requirements:</strong> @if($profile?->isSelfDriveReady())<span class="text-success">Complete</span>@else<span class="text-danger">Incomplete</span>@endif</p></div></div>@endif
            </div>
        </div>
    </form>
@endsection
